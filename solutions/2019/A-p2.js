...
function findData(matchData, matchShape, data, dataShape, byChunk = false) {
  ...
  for (let j = 0, len = data.length; j <= len - matchLen; j += step) {
    ...
    for (let i = 0; i < matchLen; ++i) {
      if (matchArray[i] !== null && matchArray[i] !== data[j + i]) {
        isEqual = false;
        break;
      }
    }
...
(async function main() {
  ...
  for (let testN = 1; testN <= totalTests; ++testN) {
    ...
    for (let d = 1; d <= minDim; ++d) {
      ...
      console.log("d=", d, result.length);

      if (max < d + d && result.length > 0) {
        max = d + d;
        maxZero = result;
      }
    }
    console.log(max, maxZero);
    ans = max;
