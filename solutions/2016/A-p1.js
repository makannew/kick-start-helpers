...
(async function main() {
  ...
  const ACharCodec="A".charCodeAt(0)
  ...
  for (let testN = 1; testN <= T; ++testN) {
    ...
    for( let i=1;i<=N;++i){
      ...
      for (let j=0,len=name.length;j<len;++j){
        if (name[j]===" ") continue
        const alphaPos = name.charCodeAt(j)-ACharCodec
        if (!L[alphaPos]){
          L[alphaPos]= true
          A[i]++
        }
    ...
    //
    let leader = 1
    let lastScore = A[leader]
    for (let i=2;i<=N;++i){
      if (A[i]>lastScore){
        leader = i
        lastScore=A[leader]
      }else if(A[i]===lastScore){
        if (names[i]<names[leader]){
          leader = i
        }
      }
    }

    printResult(testN, names[leader]);
    // functions
  }
  process.exit();
